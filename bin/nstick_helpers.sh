...
prepareStick() {
    ...
    ${debug} mkfs.ext2 -q -L casper-rw /dev/${d}2
    echo done creating ext2 file system on /dev/${d}2
}

##
## pre: file systems not mounted
## @param 1 device
## @param 2 label
function clearRoot(){
    local d=$1
...
installToStick() {
    ...
    cp /home/prepareassessment/grub/grub.cfg ${MOUNTPOINT}/boot/grub/grub.cfg
    echo 77b952cc-755f-4b75-8cfd-9089b41c1c21 > ${MOUNTPOINT}/casper-uuid-generic
...
#    ${debug} syslinux --install /dev/${d}1
    ${debug} syslinux --stupid ${BOOTPART}
     umount ${BOOTPART}
    echo update for stick ${LABEL} done
}
