...
my $svn_location= $properties->getProperty('svn_location','/exam/exam');
...
print SQL qq(
begin work;
delete from candidate_stick where stick_event_repo_id  in (select stick_event_repo_id from stick_event_repo where event='$sql_exam_id');
delete from stick_event_repo  where event='$sql_exam_id';
insert into event select '$sql_exam_id','$exam_date' where not exists (select 1 from event where event='$sql_exam_id');
);
...
# rm -fr ${local_repos_path}
# mkdir -p ${local_repos_path}/tmp
...
while ($stickcount < $examcount) {
  ...
  $projname =~ s/ /_/g;
  $reposdir='/exam/exam/'.$uname.'-repo';
  $reposurilocal='file://'.$reposdir;
  $reposuri = '/examsvn/'.$exam_id.'/'.$uname;
  $projdir='/exam/exam/Desktop/examproject-'.$uname.'/';		#.'/'.$projname;
...
* =
  ...
  print SQL qq(insert into stick_event_repo (event,stick_nr,reposroot,reposuri) values('$sql_exam_id',$sticknr,'$reposdir','$reposuri');\n);
  $stickcount++;
}
...
Use ExamSite $exam_year $exam_id $sql_exam_id
);
close(APACHECONF);
close(SETTINGSPHP);
print qq(
